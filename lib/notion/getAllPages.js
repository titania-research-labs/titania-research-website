import { idToUuid } from '@banr1/notion-utils';
...
export async function getAllPages({ allowedTypes, allowedStatuses, allowedLang = 'en' }) {
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    properties.lang = properties.lang?.[0];

    data.push(properties);
  }

  // remove all the the items doesn't meet requirements
  const posts = filterPages({ pages: data, allowedTypes, allowedStatuses, allowedLang });
